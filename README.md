...
- **Cinematic hero**: three chained AI-generated ocean film legs (surface → the great shoal → the hunt). Desktop scrubs the 16:9 1080p encodes by scroll position (blob-seek). Phones scrub too, but via pre-extracted WebP frame sequences drawn to a canvas (public/frames/leg1-3, 96 frames each at 12fps/640w), the Apple-product-page technique: deterministic on every mobile browser, no video decoder involved.
