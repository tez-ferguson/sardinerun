...
Hero legs were generated with Higgsfield Seedance 2.0 using frame-locked chaining (each leg starts from the previous leg's actual last frame — architecture A of the scroll-world method). Encodes: desktop `crf 20 -g 8` 1080p; mobile `crf 23 -g 4` 720w native portrait. Posters are each leg's first frame. See `src/components/ScrollHero.tsx` for the scrub engine.
